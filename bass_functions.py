...
import scipy
# from scipy import signal, fft # , arange redefinition of a unused imported module #UNUSED
...
import pandas as pd
# from pandas.tools.plotting import autocorrelation_plot #UNUSED
...
#from matplotlib.widgets import unicode #do NOT require in Python3 as all strings are considered unicode strings
...
#         try:
#             i = 0
#             while str(pd.read_csv('%s/%s' %(Settings['folder'], Settings['Label']), sep = '\t', index_col=False, header=None, skiprows=i, nrows=1)[0][0]).isnumeric() ==False:
#                 i += 1
#             data = pd.read_csv('%s/%s' %(Settings['folder'], Settings['Label']), sep = '\t', index_col= 0, header=None, skiprows = i)
#         except:
#             i = 0
#             while str(pd.read_csv('%s\%s' %(Settings['folder'], Settings['Label']), sep = '\t', index_col=False, header=None, skiprows=i, nrows=1)[0][0]).isnumeric() ==False:
#                 i += 1
...
#==============================================================================
    ...
    #HDF5, from RTXI, time in ms
        ...
        Settings['HDF Key'] = input('HDF Key: ')
        Settings['HDF Channel'] = input('HDF Channel (starting at 1): ')
...
def load_interact():
    ...
    Settings['folder']= input('Full File Path to Folder containing file: ')
    Settings['Label'] = input('File Name: ')
    Settings['Output Folder'] = input('Full File Path to Output Folder: ')
...
def load_settings_interact(Settings):
    ...
    Settings['Settings File'] = input('Full File path and file for the settings file: ')
    Settings = load_settings(Settings)
    return Settings
...
def user_input_trans(Settings):
    '''
    this function allows the user to specify and save their transformation settings for later analysis in an interactive way using input().
...
def savgolay_settings():
    '''
    Uses input to get settings from user for savitzky_golay
    Parameters
    ----------
    None
    Returns
...
def bandpass_settings():
    '''
    Uses input to get settings from user for butter_bandpass_filter()
    Parameters
    ----------
    None
    Returns
    ...
    temp_list = input("Bandpass Settings (lowcut, highcut,polynomial): ").lower()
...
def abs_settings():
    '''
    Uses input to get settings from user for absolute value.
    Parameters
    ----------
    None
    Returns
    ...
    abs_set = input("Absolute Value (True/False): ").lower()
...
def linear_settings(Settings):
    '''
    Uses input to get user settings for linear subtraction.
    ...
    lin_param = input("Linear Fit (True/False): ").lower()

    if lin_param == "false" or lin_param == 'none' or lin_param == '':
        Settings['Linear Fit'] = False
        return Settings
    else:
        Settings['Linear Fit'] = float(input('Linear fit R value (0-1): '))
        Settings['Linear Fit-Rolling R'] = float(input('Rolling Window Linear fit R value (0-1): '))
        Settings['Linear Fit-Rolling Window'] = int(input('Rolling Window size (even integer): '))
        Settings['Relative Baseline'] = float(input('Relative Baseline (float): '))
        return Settings
...
def user_input_base(Settings):
    '''
    this function allows the user to specify and save their baseline settings for later analysis in an interactive way using input().
    The only argument passed in and out is the Settings Dictionary.
    Parameters
    ----------
    Settings: dictionary
    ...
    baseline_type = input('Enter Linear, Rolling, or Static: ').lower()
    ...
    if baseline_type == '': #defaults to static
        ...
        Settings['Baseline Start'] = float(input('Start (seconds)'))
        if Settings['Baseline Start'] == '':
            Settings['Baseline Start'] = 0
        Settings['Baseline Stop'] = float(input('End (seconds)'))
        ...
        Settings['Rolling Baseline Window'] = float(input('Window size in seconds: '))
...
def event_peakdet_settings(Data, Settings):
    '''
    this function allows the user to specify and save their peak detection settings for later analysis in an interactive way using input().
    Parameters
    ----------
    Data: dictionary
    ...
    delta = input("Enter delta value between 0 and %s: " %round(max(Data['trans'].max())-min(Data['trans'].min()),4))
    delta = float(delta)
    ...
    peak_min = input("Enter Peak Minimum value between %s and %s: " %(round(min_data,4) ,
                                                                          round(max_data,4)))
    ...
    peak_max = input("Enter Peak Maximum value between %s and %s: " %(round(peak_min,4) ,
                                                                          round(max_data,4)))
...
def event_burstdet_settings(Data, Settings, Results):
    ...
    if Settings['Baseline Type'] == 'static':
        max_data = max(Data['trans'].max())
        min_data = min(Data['trans'].min())
        threshperc = input("Enter a threshold value between %s and %s: " %(round(min_data,4), round(max_data, 4)))
        threshperc = float(threshperc)

        Settings['Threshold'] = threshperc
    if Settings['Baseline Type'] == 'linear':
        threshperc = input("Enter a threshold proportion value between 0 and %s: " %round(max(Data['shift'].max()),4))
        threshperc = float(threshperc)
        Settings['Threshold'] = threshperc
    ...
    if Settings['Baseline Type'] == 'rolling':
        ...
        threshperc = input("Enter a threshold value between 0 and %s: " %round(max_data_rolling-max_baseline_rolling,4))
        threshperc = float(threshperc)
    ...
    cluster_time = input("Enter the minimum inter-event interval in seconds:")
    ...
    minimum_duration = input("Enter the minimum burst duration in seconds:")
    ...
    max_duration = input("Enter the maximum burst duration in seconds:")
    ...
    min_peak = input("Enter the minimum number of peaks per burst:")
    ...
    b_area = input("Do you want to calculate Burst Area? (True/False):")
    b_area = bool(b_area)
    ...
    ee = input("Do you want to Exclude Edges? (True/False):")
    ee = bool(ee)
...
def burstarea(data, time, burst_start, burst_end, dx = 10):
    ...
    """
    from scipy.integrate import trapz #, simps #unused imported module #import the integral functions
...
def frequency_plot(event_type, meas, key, Data, Settings, Results):
    ...
    if event_type.lower() == 'peaks':
        ...
        #columns = Results['Bursts-Master'] #never used variable
    else:
        raise ValueError('Not an acceptable event type measurement.\n Must be "Peaks" or "Bursts" ')
...
def analyze(Data, Settings, Results):
    ...
    redo = input("Do you wish to re-run peak and burst detection? (y/n) ")
    redo = redo.lower()
