from bass_functions import load_wrapper, load_settings
from modules import pleth_analysis
...
class BASS_Dataset(object):
    ...
    def run_analysis(self, analysis_mod, settings = None, batch = True):
        '''Runs in either single (batch=False) or batch mode. Assuming batch mode, this function first syncs settings of each dataset within Bass_Dataset.Batch to the entered parameter "settings", then runs analysis on each instance within Batch.
